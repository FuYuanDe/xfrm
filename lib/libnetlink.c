...
///创建套接字，SOCK_CLOEXEC用于执行exec函数时关闭相关的文件描述符
/// 这里处理rth->fd
...
///设置本端netlink套接字地址，没有设置pid，此处应该为内核自动配置。
///传入的subscriptions值为0，单播
...
/// rtnl_dump_filter_arg 结构体封装了处理函数和一个备用参数指针
...
/// rtnl_dump_filter_nc(&rth, xfrm_state_print, stdout,0)
/// rtnl_dump_filter_l(struct renl_handler,	const struct renl_dump_filter_arg*)
/// 这里第二个参数filter是一个函数指针
int rtnl_dump_filter_nc(struct rtnl_handle *rth,
		     rtnl_filter_t filter,
		     void *arg1, __u16 nc_flags)
{
/// 注意函数指针filter又被封装到了结构体rtnl_dump_filter_arg中
...
///接收内核返回消息
        ...
        printf("DEBUG:at %s %d,recvmsg length %d\n",__FUNCTION__,__LINE__,status);
...
///
///	addattr_l(&req.n, sizeof(req.buf), XFRMA_REPLAY_ESN_VAL,&replay_esn, sizeof(replay_esn));
...
/// 获取NLMSG消息尾部payload位置指针
	rta = NLMSG_TAIL(n);
/// 这里payload就是rta，设置类型、长度等值。	
	rta->rta_type = type;
	rta->rta_len = len;
/// 将数据部分复制过来	
	memcpy(RTA_DATA(rta), data, alen);
/// 在这一步骤中nlmsg长度扩增以增加消息内容	
...
{
/// 看起来type像是一个index啊，为什么不直接叫index呢
	unsigned short type;
/// 初始化表格
