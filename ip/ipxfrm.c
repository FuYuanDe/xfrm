...
int xfrm_parse_mark(struct xfrm_mark *mark, int *argcp, char ***argvp)
{
	...
	NEXT_ARG();
/// 判断argv mark值是否合法，之后将其赋值给mark->v	
...
/// xfrm_id_parse(&req.xsinfo.saddr, &req.xsinfo.id,&req.xsinfo.family, 0, &argc, &argv);
...
/// 为什么要去设置全局变量xfrm_filter类型的filter
/// 可能在某个地方用到
...
/// 如果没有proto或者缺少任何一个(src，dst, spi等)
/// 可能不同的调用函数会传来不同的losse值
...
/// get_u32 用于将字符串转换成unsigned int，这个函数值得学习。
